...
import { executeWhere, executeWhereQuery, } from "./executeWhere.js";
import { executeWhy } from "./executeWhy.js";
import { getAction } from "./actionRegistry.js";
import { evaluateGraphQuery } from "./evaluateGraphControl.js";
export function executeQuery(graph, query, bindings, actions, workspace) {
    switch (query.type) {
        case "MatchExpr":
        ...
        case "WhyExpr":
            return executeWhy(graph, query, workspace);
        case "HowExpr":
            return executeHow(query, bindings, actions);
        case "WhereExpr":
            return executeWhereExpr(graph, query, bindings);
        case "GraphQueryExpr":
            return executeGraphQueryExpr(graph, query, bindings, actions);
        default: {
...
function executeGraphQueryExpr(graph, query, bindings, actions) {
    return {
        kind: "BooleanQueryResult",
        value: evaluateGraphQuery(graph, query, { bindings, actions }),
    };
}
...
export function executeMatch(graph, query, runtimeBindings) {
    const base = {
        kind: "MatchResultSet",
