export function executeWhere(graph, source, condition) {
...
export function executeWhereQuery(graph, condition, bindings) {
...
function evaluateWhereTargetExpr(expr, graph, context, item, bindings) {
    switch (expr.type) {
        case "BinaryBooleanExpr":
...
function evaluateWhereBinaryBoolean(expr, graph, context, item, bindings) {
    if (expr.operator === "&&") {
        return (evaluateWhereTargetExpr(expr.left, graph, context, item, bindings) &&
...
function evaluateWhereComparison(expr, graph, context, item, bindings) {
...
function resolveWhereBooleanValue(value, graph, context, item, bindings) {
    switch (value.type) {
        case "Identifier":
...
function evaluateWhereDeriveValue(expr, graph, bindings) {
    switch (expr.type) {
        case "DeriveStateExpr":
...
function evaluateWhereTargetDeriveValue(expr, graph, context, item, bindings) {
    const asBindings = {};
    ...
    if (context === "node") {
        asBindings.node = whereNodeToValue(item);
    }
    else {
        asBindings.edge = whereEdgeToValue(item);
    }
    return evaluateWhereDeriveValue(expr, graph, asBindings);
}
function evaluateWhereDeriveOperand(expr, graph, bindings) {
    switch (expr.type) {
        case "StringLiteral":
            return expr.value;
        case "NumberLiteral":
...
function readDerivedLayer(graph, bindings, nodeRef, layer, key) {
    if (!nodeRef || !key) {
...
function evaluateWhereDerivePath(expr, graph, bindings) {
    ...
    for (let depth = 0; depth < maxDepth; depth += 1) {
        ...
        for (const currentNodeId of frontier) {
            for (const nextNodeId of collectWherePathNeighbors(graph, currentNodeId, relations, direction)) {
    ...
    return nodeIds.filter((candidateId) => evaluateWherePathWhereExpr(graph, candidateId, expr.where, bindings));
}
function evaluateWhereDeriveCollect(expr, graph, bindings) {
...
function evaluateWhereDeriveSum(expr, graph, bindings) {
    if (expr.from) {
        return evaluateWhereFieldAggregate(expr.from, expr.field, graph, bindings, "@derive.sum", "sum");
    }
    if (!expr.collect) {
...
function evaluateWhereFieldAggregate(source, field, graph, bindings, opName, mode) {
    if (!source) {
...
function evaluateWhereAggregateSource(source, graph, bindings) {
...
function resolveWhereAggregateFieldValue(node, field) {
...
function resolveWherePathRelations(relation) {
...
function collectWherePathNeighbors(graph, nodeId, relations, direction) {
...
function countDerivedEdges(graph, bindings, nodeRef, relation, direction, where = null) {
    ...
    const matchingEdges = graph.edges.filter((edge) => {
        if (edge.relation !== relation)
            return false;
        if (direction === "incoming" && edge.object !== nodeId)
            return false;
        if (direction === "outgoing" && edge.subject !== nodeId)
            return false;
        if (!where)
            return true;
...
function evaluateWhereNumericBinary(operator, left, right) {
    if (typeof left !== "number" || typeof right !== "number") {
...
function printDeriveSourceExpr(expr) {
...
function printDeriveOperand(expr) {
    switch (expr.type) {
        case "CurrentValue":
            return expr.name;
        case "PreviousValue":
...
function printDeriveRelation(value) {
    if (value.type === "StringLiteral") {
        return value.raw;
    }
    return `[${value.elements
        .map((element) => element.type === "StringLiteral" ? element.raw : JSON.stringify(element))
        .join(", ")}]`;
}
function evaluateWherePathWhereExpr(graph, nodeId, where, bindings) {
    const node = graph.nodes.get(nodeId);
    if (!node)
        return false;
