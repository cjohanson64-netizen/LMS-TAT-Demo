import { getIncomingEdges, getOutgoingEdges, } from "./graph.js";
...
export function compareGenealogyRelationship(graph, request) {
    ...
    const selectedCommonAncestorId = sharedAncestors.some((ancestor) => ancestor.id === request.selectedCommonAncestorId)
        ? request.selectedCommonAncestorId
        : undefined;
    if (from.id === to.id) {
        return buildComparisonResult(from, to, {
            type: "self",
            label: "self",
        }, [from.id], [], emptyHighlight(), availableCommonAncestors, selectedCommonAncestorId);
    }
    if (areSpouses(graph, from.id, to.id)) {
        ...
        ], emptyHighlight(), availableCommonAncestors, selectedCommonAncestorId);
    }
    const stepParentPath = findStepParentBridgePath(graph, from.id, to.id) ??
        findDirectParentChildPath(graph, from.id, to.id, null, PRIMARY_PARENT_RELATION, isStepParentEdge);
    if (stepParentPath) {
        return buildComparisonResult(from, to, {
            type: "stepParent",
            label: "step-parent",
        }, stepParentPath.nodeIds, stepParentPath.edges, emptyHighlight(), availableCommonAncestors, selectedCommonAncestorId);
    }
    const stepChildPath = findStepChildBridgePath(graph, from.id, to.id) ??
        findDirectParentChildPath(graph, to.id, from.id, null, PRIMARY_PARENT_RELATION, isStepParentEdge);
    if (stepChildPath) {
        return buildComparisonResult(from, to, {
            type: "stepChild",
            label: "step-child",
        }, stepChildPath.nodeIds, stepChildPath.edges, emptyHighlight(), availableCommonAncestors, selectedCommonAncestorId);
    }
    const ancestorPath = findShortestRelationshipPath(graph, to.id, from.id, "incoming");
    if (ancestorPath) {
        return buildComparisonResult(from, to, {
            type: "ancestor",
            label: formatAncestorLabel(ancestorPath.depth),
            depth: ancestorPath.depth,
        }, ancestorPath.nodeIds, ancestorPath.edges, emptyHighlight(), availableCommonAncestors, selectedCommonAncestorId);
    }
    const descendantPath = findShortestRelationshipPath(graph, to.id, from.id, "outgoing");
    if (descendantPath) {
        return buildComparisonResult(from, to, {
            type: "descendant",
            label: formatDescendantLabel(descendantPath.depth),
            depth: descendantPath.depth,
        }, descendantPath.nodeIds, descendantPath.edges, emptyHighlight(), availableCommonAncestors, selectedCommonAncestorId);
    }
    const siblingPath = findSiblingPath(graph, from.id, to.id);
    if (siblingPath) {
        return maybeOverrideWithSelectedCommonAncestor(graph, {
            from,
            to,
            relationship: {
                type: "sibling",
            ...
            highlight: buildSplitHighlight([from.id], [siblingPath.edges[0]].filter(Boolean), [siblingPath.nodeIds[1]].filter(Boolean), [to.id], [siblingPath.edges[1]].filter(Boolean)),
        }, selectedCommonAncestorId, availableCommonAncestors);
    }
    ...
    if (auntUnclePath) {
        return maybeOverrideWithSelectedCommonAncestor(graph, {
            ...
            highlight: buildSplitHighlight([from.id], [auntUnclePath.edges[0]].filter(Boolean), auntUnclePath.nodeIds[1] ? [auntUnclePath.nodeIds[1]] : [], auntUnclePath.nodeIds.slice(2), auntUnclePath.edges.slice(1)),
        }, selectedCommonAncestorId, availableCommonAncestors);
    }
    ...
    if (nieceNephewPath) {
        return maybeOverrideWithSelectedCommonAncestor(graph, {
            ...
            highlight: buildSplitHighlight([from.id, nieceNephewPath.nodeIds[1]].filter(Boolean), nieceNephewPath.edges.slice(0, 2), nieceNephewPath.nodeIds[2] ? [nieceNephewPath.nodeIds[2]] : [], nieceNephewPath.nodeIds.slice(3, 4), nieceNephewPath.edges.slice(2)),
        }, selectedCommonAncestorId, availableCommonAncestors);
    }
    const cousinPath = findCousinPath(graph, from.id, to.id, selectedCommonAncestorId);
    if (cousinPath) {
        return buildComparisonResult(from, to, {
            type: "cousin",
        ...
        }, cousinPath.nodeIds, cousinPath.edges, cousinPath.highlight, availableCommonAncestors, cousinPath.selectedCommonAncestorId, {
    ...
    if (parentInLawPath) {
        return buildComparisonResult(from, to, {
            type: "parentInLaw",
            label: "parent-in-law",
        }, parentInLawPath.nodeIds, parentInLawPath.edges, emptyHighlight(), availableCommonAncestors, selectedCommonAncestorId);
    }
    ...
    if (childInLawPath) {
        return buildComparisonResult(from, to, {
            type: "childInLaw",
            label: "child-in-law",
        }, [from.id, childInLawPath.nodeIds[1], to.id], childInLawPath.edges, emptyHighlight(), availableCommonAncestors, selectedCommonAncestorId);
    }
    return buildComparisonResult(from, to, {
        type: "unknown",
...
function buildComparisonResult(from, to, relationship, pathNodeIds, pathEdges, highlight, availableCommonAncestors, selectedCommonAncestorId, sharedAncestor, depths) {
    return {
        from,
        to,
        relationship,
...
function maybeOverrideWithSelectedCommonAncestor(graph, baseResult, selectedCommonAncestorId, availableCommonAncestors) {
    if (!selectedCommonAncestorId) {
        return buildComparisonResult(baseResult.from, baseResult.to, baseResult.relationship, baseResult.pathNodeIds, baseResult.pathEdges, baseResult.highlight, availableCommonAncestors, undefined);
    }
    const overridePath = buildCommonAncestorOverridePath(graph, baseResult.from.id, baseResult.to.id, selectedCommonAncestorId);
    if (!overridePath) {
        return buildComparisonResult(baseResult.from, baseResult.to, baseResult.relationship, baseResult.pathNodeIds, baseResult.pathEdges, baseResult.highlight, availableCommonAncestors, undefined);
    }
    return buildComparisonResult(baseResult.from, baseResult.to, baseResult.relationship, overridePath.pathNodeIds, overridePath.pathEdges, overridePath.highlight, availableCommonAncestors, selectedCommonAncestorId);
}
...
function buildSplitHighlight(selectedPathNodeIds, selectedPathEdges, commonNodeIds, targetPathNodeIds, targetPathEdges) {
    const commonNodeIdSet = new Set(commonNodeIds);
    const selectedNodeIdSet = new Set(selectedPathNodeIds.filter((nodeId) => !commonNodeIdSet.has(nodeId)));
    const targetNodeIdSet = new Set(targetPathNodeIds.filter((nodeId) => !commonNodeIdSet.has(nodeId) && !selectedNodeIdSet.has(nodeId)));
    const selectedEdges = uniqueEdges(selectedPathEdges);
    const targetEdges = uniqueEdges(targetPathEdges).filter((edge) => !selectedEdges.some((selectedEdge) => sameEdge(selectedEdge, edge)));
    return {
...
function findDirectParentChildPath(graph, parentId, childId, relations, relationLabel, predicate) {
    const matchingEdge = getOutgoingEdges(graph, parentId, "branch").find((edge) => edge.object === childId &&
        (predicate
            ? predicate(edge)
            : relations
...
function findStepParentBridgePath(graph, stepParentId, childId) {
    const explicitStepEdge = getOutgoingEdges(graph, stepParentId, "branch").find((edge) => isStepParentEdge(edge) && edge.object === childId);
    if (!explicitStepEdge) {
        return null;
    }
    const spouseBridgeId = getSpouseIds(graph, stepParentId).find((spouseId) => getOutgoingEdges(graph, spouseId, "branch").some((edge) => isBirthParentEdge(edge) && edge.object === childId));
    if (!spouseBridgeId) {
        return null;
    }
    return {
...
function findStepChildBridgePath(graph, childId, stepParentId) {
...
export function queryGenealogyCommonAncestors(graph, request) {
    const fromNode = graph.nodes.get(request.fromId);
    const toNode = graph.nodes.get(request.toId);
    if (!fromNode || !toNode) {
    ...
    return {
        ...
        ancestors: sharedAncestors.map(({ fromDepth: _fromDepth, toDepth: _toDepth, ...ancestor }) => ancestor),
    };
}
function collectSharedAncestors(graph, fromId, toId) {
    ...
    return [...fromAncestorDepths.keys()]
        ...
        .sort((left, right) => {
        const depthDelta = left.fromDepth + left.toDepth - (right.fromDepth + right.toDepth);
        if (depthDelta !== 0) {
            return depthDelta;
        }
        const maxDepthDelta = Math.max(left.fromDepth, left.toDepth) -
...
function findShortestRelationshipPath(graph, startNodeId, targetNodeId, direction) {
    ...
    while (queue.length > 0) {
        ...
        for (const edge of edges) {
            ...
            const nextNodeId = direction === "incoming" ? edge.subject : edge.object;
            if (visited.has(nextNodeId)) {
                continue;
            }
            const nextDepth = current.depth + 1;
            const pathEdge = direction === "incoming"
                ? {
                    from: nextNodeId,
                    relation: PRIMARY_PARENT_RELATION,
                    to: current.nodeId,
...
function areSpouses(graph, fromId, toId) {
    return (getOutgoingEdges(graph, fromId, "branch").some((edge) => isSpouseEdge(edge) && edge.object === toId) ||
        getIncomingEdges(graph, fromId, "branch").some((edge) => isSpouseEdge(edge) && edge.subject === toId));
}
function findSiblingPath(graph, fromId, toId) {
    if (fromId === toId) {
        return null;
    }
    ...
    const toParents = new Set(getIncomingEdges(graph, toId, "branch")
        .filter((edge) => isBirthParentEdge(edge))
        .map((edge) => edge.subject));
...
function findAuntUnclePath(graph, fromId, toId) {
...
function findCousinPath(graph, fromId, toId, selectedCommonAncestorId) {
    const candidate = selectCousinAncestor(graph, fromId, toId, selectedCommonAncestorId);
    if (!candidate) {
        return null;
    }
    const overridePath = buildCommonAncestorOverridePath(graph, fromId, toId, candidate.id);
    if (!overridePath) {
        return null;
    }
...
function selectCousinAncestor(graph, fromId, toId, selectedCommonAncestorId) {
    const eligibleAncestors = collectSharedAncestors(graph, fromId, toId).filter((ancestor) => ancestor.fromDepth >= 2 && ancestor.toDepth >= 2);
    if (eligibleAncestors.length === 0) {
        return null;
    }
    if (selectedCommonAncestorId) {
        const selectedAncestor = eligibleAncestors.find((ancestor) => ancestor.id === selectedCommonAncestorId);
        if (selectedAncestor) {
...
function findNieceNephewPath(graph, fromId, toId) {
...
function findParentInLawPath(graph, fromId, toId) {
...
function getSpouseIds(graph, nodeId) {
    return getOutgoingEdges(graph, nodeId, "branch")
        .filter((edge) => isSpouseEdge(edge))
        .map((edge) => edge.object);
}
function getParentIds(graph, nodeId) {
    return getIncomingEdges(graph, nodeId, "branch")
        .filter((edge) => isBirthParentEdge(edge))
        .map((edge) => edge.subject);
}
function collectAncestorDepths(graph, startNodeId) {
    const visited = new Set([startNodeId]);
...
function buildCommonAncestorOverridePath(graph, fromId, toId, ancestorId) {
    ...
    let commonLength = 0;
    while (commonLength < fromDescPath.length &&
...
function findUpwardBirthAncestorPath(graph, startNodeId, ancestorId) {
...
function buildDescendantEdges(nodeIds) {
    if (nodeIds.length < 2) {
        return [];
    }
    const result = [];
...
function formatAncestorLabel(depth) {
    if (depth === 1)
        return "parent";
    if (depth === 2)
        return "grandparent";
...
function formatDescendantLabel(depth) {
    if (depth === 1)
        return "child";
    if (depth === 2)
        return "grandchild";
...
function formatCousinLabel(degree, removed) {
...
function formatNieceNephewLabel(node) {
    const value = isRecord(node.value) ? node.value : null;
