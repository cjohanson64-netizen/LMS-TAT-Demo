...
private parseGraphPipelineStep(): GraphPipelineStepNode {
  ...
  ): { from: DeriveAggregateSourceNode | null; field: StringLiteralNode | null } {
    let from: DeriveAggregateSourceNode | null = null;
    let field: StringLiteralNode | null = null;
  ...
  private parseNamedArguments(
    name: string,
  ): Array<{ key: IdentifierNode; value: ValueExprNode | AggregateQueryExprNode | DeriveExprNode }> {
    const fields: Array<{ key: IdentifierNode; value: ValueExprNode | AggregateQueryExprNode | DeriveExprNode }> = [];
    this.skipNewlines();
    while (!this.checkType("RPAREN")) {
      this.skipNewlines();
      if (!(this.checkType("IDENT") && this.peekNext().type === "COLON")) {
  ...
  private parseDeriveAggregateSourceValue(
    ...
    } as Token : this.peek(), "Expected @derive.path or @query aggregate source");
  }
  ...
  private parseDeriveExistsValue(
    ...
    throw this.error(value.span ? {
      ...
      column: value.span.column,
      index: value.span.start,
    } as Token : this.peek(), "Expected @derive.path or identifier source");
  }
  ...
  private parseStringLiteralValue(
    ...
    throw this.error(value.span ? {
      ...
      column: value.span.column,
      index: value.span.start,
    } as Token : this.peek(), `${opName} ${fieldName} must be a string literal`);
  }
  ...
  private parseDeriveCollectValue(
    ...
    throw this.error(value.span ? {
      ...
      column: value.span.column,
      index: value.span.start,
    } as Token : this.peek(), "Expected @derive.collect value");
  }
